...
void main() {
  ...
  // aurora only shows on some days (dayFactor=1 hides it)
  #ifdef NL_AURORA
    float auroraDayFactor = mix(1.0, env.dayFactor, nlAuroraDay(Day.x));
  #else
    float auroraDayFactor = env.dayFactor;
  #endif
  ...
  vec3 worldPos;
    ...
    vec4 color;
      ...
      float fade = fog_fade(worldPos.xyz);
        ...
        #ifdef NL_AURORA
          color += renderAurora(cloudPos, t, rain, auroraDayFactor)*(1.0-color.a);
        #endif

        color.a *= fade;
        color.rgb = colorCorrection(color.rgb);
      #else // NL_CLOUD_TYPE 2
        v_dayFactor = auroraDayFactor;
