...
vec3 colorCorrection(vec3 col) {
  ...
  // contrast around mid-grey (deeper shadows, clearer lit areas)
  col = max((col - 0.45)*NL_CONTRAST + 0.45, vec3_splat(0.0));
  ...
  // pseudo-bloom: cheap highlight bleed (not a real blur pass)
  #ifdef NL_BLOOM_STRENGTH
    col += NL_BLOOM_STRENGTH*col*smoothstep(0.65, 1.3, luminance(col));
  #endif
